...
def timezone_to_posix(timezone: str | None) -> str | None:
    """
    Convert a timezone string to its POSIX equivalent.
    Returns None if the timezone is not found in the database.
    """
    if timezone is None:
        return None
    return TIMEZONE_TO_POSIX.get(timezone, None)